...
export const login = (email, password) => {
	return (dispatch, getState) => {
		...
		axios
			.post(
				"https://nscc.org.ng/wp-json/jwt-auth/v1/token",
				{
					username: email,
					password: password,
				},
	...
	return async (dispatch, getState) => {
		...
		try {
			...
			if (beers.length > 0) {
				 ...
				 * Send the beers data.
				 */
				const beersCall = await pushAjax("nscc_blog", token, beers);
				dispatch(pushSuccess("blogs", beers, beersCall.data));
...
export const pushAjax = async (type, token, data) => {
	if (type === "photos") {
		...
		return axios
			.post(
				"https://nscc.org.ng/wp-json/nscc_blog/v1/push",
				{
					type,
					data,
				},
...
export const validateLoginAjax = async token => {
	return axios
		.post(
			"https://nscc.org.ng/wp-json/jwt-auth/v1/token/validate",
			{},
			{
				headers: { Authorization: `Bearer ${token}` },
				timeout: 10000,
...
export const pullData = () => {
	return async (dispatch, getState) => {
		...
		try {
			 ...
			 */
			const beersCall = await pullAjax("nscc_blog", userId, token);
			dispatch(pullSuccess("blogs", beersCall.data));
...
export const pullAjax = async (type, userId, token) => {
	return axios
		.post(
			"https://nscc.org.ng/wp-json/jwt-auth/nscc_blog/v1/pull",
			{
				type,
				user_id: userId,
			},
